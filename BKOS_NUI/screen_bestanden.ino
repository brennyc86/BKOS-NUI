...
static void _bf_scan() {
    ...
    while (f && bf_cnt < BF_MAX) {
        strncpy(bf_lijst[bf_cnt].naam, f.name(), sizeof(bf_lijst[bf_cnt].naam) - 1);
...
static bool _bf_verwijder(int i) {
    ...
    bool ok;
#if BF_SD_MOGELIJK
    if (bf_toont_sd) ok = SD.remove(bf_lijst[i].naam);
    else             ok = SPIFFS.remove(bf_lijst[i].naam);
#else
    ok = SPIFFS.remove(bf_lijst[i].naam);
#endif
    if (ok) _bf_scan();
    return ok;
}
...
void screen_bestanden_teken() {
    ...
    tft.setCursor(10, CONTENT_Y + (BF_HDR_H - 16) / 2); tft.print("BESTANDEN");
    ...
    if (bf_cnt == 0) {
        ...
        for (int i = 0; i < bf_cnt; i++) {
            ...
            tft.print(naam);
            if (bf_lijst[i].map) tft.print("/");

            char gb[16]; _bf_fmt_bytes(bf_lijst[i].bytes, gb, sizeof(gb));
            tft.setTextColor(C_TEXT_DIM);
            tft.setCursor(14, ry + 20); tft.print(bf_lijst[i].map ? "map" : gb);
...
void screen_bestanden_run(int x, int y, bool aanraking) {
    ...
    if (y < y0) return;
    int i = (y - y0) / BF_ROW_H;
    if (i < 0 || i >= bf_cnt || bf_lijst[i].map) return;
