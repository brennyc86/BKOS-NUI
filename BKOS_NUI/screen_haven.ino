...
static void _hv_app_icoon_teken(int icoon, int cx, int cy, float s, uint16_t kleur) {
    switch (icoon) {
        case I_USB: {
            ...
            // gaatje meer); driehoek een fractie kleiner dan de vorige ronde
            // en meer hoogteverschil tussen de 3 takken. Vierkant (rechts)
            // nu een fractie lager, en cirkel (links) heeft er zijn eigen
            // korte rechte stukje bij gekregen (schuin naar het vertakpunt,
            // dan recht omhoog naar de cirkel) — zo ziet het echte logo er
            // ook uit — en komt daardoor lager uit dan het vierkant.
            int o2 = max(2, _hv_app_ic_o(2, s)), o3 = _hv_app_ic_o(3, s),
                o4 = _hv_app_ic_o(4, s), o5 = _hv_app_ic_o(5, s),
                o7 = _hv_app_ic_o(7, s), o9 = _hv_app_ic_o(9, s);
            int o_tri = _hv_app_ic_o(5, s) / 2;   // driehoek: tussen o2 en o3 in, kleiner dan de vorige ronde (was o3)
            ...
            tft.drawLine(cx, cy + o2, cx - o4, cy - o2, kleur);            // linkertak (laagste): schuin...
            tft.drawFastVLine(cx - o4, cy - o3, o3 - o2, kleur);           // ...dan een kort stukje recht omhoog naar de cirkel
            tft.fillCircle(cx - o4, cy - o3 - r_zij, r_zij, kleur);        // ...die er precies op aansluit

            tft.drawFastVLine(cx, cy - o9, o9 + o2, kleur);                // middentak (hoogste): recht omhoog
            tft.fillTriangle(cx - o_tri, cy - o9, cx + o_tri, cy - o9, cx, cy - o9 - o_tri, kleur);  // driehoek, sluit aan op de lijn

            tft.drawLine(cx, cy + o2, cx + o4, cy - o2, kleur);            // rechtertak (middelste hoogte, iets lager): schuin...
        ...
        case I_230V: {
            // Echte "Z"-bliksemschicht — vorige poging (met "nette", ronde
            // eigen coördinaten) brak de zigzag en oogde als een ruit. Deze
            // punten zijn rechtstreeks herleid uit een bestaand, beproefd
            // bliksem-icoon-pad (Bootstrap Icons "lightning", herschaald),
            ...
            int p1x = cx - top_half, p1y = cy - o9 + 5;   // platte top, links — 5px lager: echte bovenkant loopt nu een beetje schuin
            int p2x = cx + top_half, p2y = cy - o9;       // platte top, rechts
            int p3x = cx + o1, p3y = cy - o2;   // keep 1 (bovenste)
            int p4x = cx + o4, p4y = cy - o2;   // sprong bij keep 1
            int p5x = cx - o2, p5y = cy + o9;   // spitse punt onderaan
            int p6x = cx - o1, p6y = cy + o2;   // keep 2 (onderste)
            int p7x = cx - o4, p7y = cy + o2;   // sprong bij keep 2
            uint16_t licht  = RGB565(255, 240, 150);
            uint16_t donker = RGB565(200, 130, 10);
            tft.fillTriangle(p3x, p3y, p4x, p4y, p5x, p5y, kleur);
            tft.fillTriangle(p3x, p3y, p5x, p5y, p6x, p6y, kleur);
            tft.fillTriangle(p3x, p3y, p6x, p6y, p7x, p7y, kleur);
            tft.fillTriangle(p3x, p3y, p7x, p7y, p1x, p1y, kleur);
            tft.fillTriangle(p3x, p3y, p1x, p1y, p2x, p2y, kleur);
            tft.drawLine(p2x, p2y, p3x, p3y, licht);   // rechterflank boven = glans
            tft.drawLine(p5x, p5y, p6x, p6y, donker);  // linkerflank onder = schaduw
            break;
        }
        case I_TV: {
            // Kast als 2 dikke (~5pt) lagen exact over elkaar — één afgerond,
            // één scherp — plus dikkere (2-3px) konijnenoren-antenne, met wat
            // luchtruimte tussen de sprieten en de bovenkant van de tegel
            // (was: te strak tegen de rand). Voet nu een trapezium (~9px
            // onder, ~6px boven — vaste pixelbreedtes, niet schalend met s)
            // i.p.v. een rechte balk, en een fractie hoger/korter.
            int dik = max(2, _hv_app_ic_o(2, s));    // ~5pt kaderdikte
            int o3 = _hv_app_ic_o(3, s),
                o4 = _hv_app_ic_o(4, s), o5 = _hv_app_ic_o(5, s),
                o7 = _hv_app_ic_o(7, s), o8 = _hv_app_ic_o(8, s), o11 = _hv_app_ic_o(11, s),
                o16 = _hv_app_ic_o(16, s);
            int bx = cx - o8, by = cy - o5;
            // Antenne-tip nu op o10 (was o12/o13) — nog wat korter, de vorige
            // marge tot de bovenkant van de tegel was nog niet genoeg (leek
            // afgesneden/weg).
            int o10 = _hv_app_ic_o(10, s);
            for (int d = -1; d <= 1; d++) {                                // antenne, 3x geteken (±1px) voor meer dikte
                tft.drawLine(cx - o3 + d, by, cx - o7 + d, cy - o10, kleur);
                tft.drawLine(cx + o3 + d, by, cx + o7 + d, cy - o10, kleur);
            }
            ...
            // Bug: voet_top_y stond hierboven op cy+o5, wat BOVEN de werkelijke
            // onderkant van de kast (by+o11) ligt — de voet begon dus binnen
            ...
            int tv_onder = by + o11;
            // Voet nu gekoppeld aan de kastbreedte (o16) i.p.v. vaste
            // pixelbreedtes — bijna zo breed als de binnenkant van de kast
            // (gevraagd), taps naar een smallere hals bovenaan.
            int bot_w = max(6, o16 - 2 * dik - 2);
            int top_w = max(4, bot_w * 3 / 5);
            ...
            // Horizontale voet — een liggende balk net onder het pootje,
            // iets breder dan de onderkant ervan.
            int voet_w = bot_w + o3;
            for (int i = 0; i < dik; i++)
        ...
        case I_WATER: {
            // Maan-accenten nu als ring-segment (tft.fillArc(), zelfde
            // primitief als de AUTO-knop in screen_main.ino) tussen een
            // binnen- en buitenstraal die allebei BINNEN de druppelrand
            // blijven. Dat geeft automatisch, van buiten naar binnen: een
            // randje druppelkleur, dan het maantje, dan weer druppelkleur in
            // het gat — precies gevraagd, en betrouwbaarder dan de vorige
            ...
            // Driehoek-basis op o5+o1 sloot mooi aan, maar bleek een fractie
            // te breed (~3px, vast getal) — stak net iets uit voorbij de bol.
            int driehoek_breedte = max(o2 + 1, o5 + o1 - 3);
            tft.fillTriangle(cx, cy - o9, cx - driehoek_breedte, cy + o2, cx + driehoek_breedte, cy + o2, kleur);
...
void screen_haven_teken() {
    ...
    // "Scan nog bezig"-melding zit nu in het lichtere hoekje rechtsonder van
    // de achtergrondfoto zelf (haven_achtergrond.ino: _hab_hoek_teken()) —
    // geen los tekstlabel hier meer nodig. Paneel/tegels blijven gewoon
    // bedienbaar; alleen de foto verspringt straks nog één keer.
